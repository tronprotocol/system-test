...
@Setter
public class UnDelegateResourceCreator extends AbstractTransactionCreator implements GoodCaseTransactonCreator {
  ...
  protected Protocol.Transaction create() {
    ...
    Contract.UnDelegateResourceContract contract;
    if(createFreezeBalanceV2Count.get() % 2 == 0L) {
      TransactionFactory.context.getBean(CreatorCounter.class).put(this.getClass().getName());
      contract = unDelegateResourceContract(getAddress(privateKey),
          new Random().nextInt(500000) + 500000,
    ...
    Protocol.Transaction transaction = createTransaction(contract, ContractType.UnDelegateResourceContract);
