...
@Slf4j
@MultiNode
public class ContractInternalTransaction001 extends TronBaseTest {
  ...
  public void beforeClass() {
    initSolidityChannel();
    PublicMethod.printAddress(testKeyForinternalTxsAddress);
  ...
  @Test(
      enabled = true,
      description = "Create->call.Two-level nesting",
  ...
  @Test(
      enabled = true,
      description = "There is one internalTransaction.Only call",
      groups = {"contract", "daily"})
  public void testInternalTransaction002() {
    ...
    Assert.assertEquals(
        1, infoById1.get().getInternalTransactions(0).getCallValueInfo(0).getCallValue());
  }

  @Test(
      enabled = true,
      description = "There is one internalTransaction.Only create",
      groups = {"contract", "daily"})
  public void testInternalTransaction003() {
    ...
    Assert.assertEquals(
        10, infoById1.get().getInternalTransactions(0).getCallValueInfo(0).getCallValue());
  }

  @Test(
      enabled = true,
      description = "Test suicide type in internalTransaction",
      groups = {"contract", "daily"})
  public void testInternalTransaction004() {
    ...
    String note5 =
        ByteArray.toStr(infoById1.get().getInternalTransactions(2).getNote().toByteArray());
    ...
    for (int i = 0; i < transactionsCount1; i++) {
      Assert.assertFalse(infoById1.get().getInternalTransactions(i).getRejected());
      Assert.assertEquals(
          0, infoById1.get().getInternalTransactions(i).getCallValueInfo(0).getCallValue());
    }
  }

  @Test(
      enabled = true,
      description = "Type is create call",
      groups = {"contract", "daily"})
  ...
  @Test(
      enabled = true,
      description = "Type is create call call",
      groups = {"contract", "daily"})
