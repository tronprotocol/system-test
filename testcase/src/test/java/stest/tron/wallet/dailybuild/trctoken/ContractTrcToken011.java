...
@Slf4j
@MultiNode
public class ContractTrcToken011 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "TransferToken with correct value, deploy transfer contract",
  ...
  public void test01DeployTransferTokenContract001() {
    ...
    // PublicMethod.randomFreezeAmount.getAndAdd(1),
    ...
    Account getAssetIdFromThisAccount = PublicMethod.queryAccount(dev001Address, blockingStubFull);
    ...
    SmartContract smartContract =
        PublicMethod.getContract(transferTokenContractAddress, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());

    /*Assert.assertFalse(PublicMethod.transferAsset(transferTokenContractAddress,
  ...
  @Test(
      enabled = true,
      description = "TransferToken with correct value, deploy receive contract",
      groups = {"contract", "daily"})
  public void test02DeployRevContract002() {
    ...
    Optional<TransactionInfo> infoById =
        PublicMethod.getTransactionInfoById(recieveTokenTxid, blockingStubFull);
    logger.info("Deploy energytotal is " + infoById.get().getReceipt().getEnergyUsageTotal());
    if (infoById.get().getResultValue() != 0) {
    ...
    SmartContract smartContract = PublicMethod.getContract(resultContractAddress, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());
    Long contractAssetCount =
        PublicMethod.getAssetIssueValue(resultContractAddress, assetAccountId, blockingStubFull);
    logger.info(
        "Contract has AssetId: "
            + assetAccountId.toStringUtf8()
            + ", Count: "
  ...
  @Test(
      enabled = true,
      description = "TransferToken with correct value, transfer to a contract",
      groups = {"contract", "daily"})
  ...
  @Test(
      enabled = true,
      description = "TransferToken with correct value, get contract tokenBalance",
      groups = {"contract", "daily"})
