 ...
 *
 * <p>Tests the dynamic energy pricing mechanism where frequently-called contracts pay higher energy
 * costs to prevent resource abuse.
 */
@Slf4j
public class DynamicEnergyTest002 extends TronBaseTest {
  ...
  public void beforeClass() throws Exception {
    ...
    Assert.assertTrue(
        PublicMethod.freezeBalanceGetEnergy(
            testAddress, 50 * TronConstants.THOUSAND_TRX, 0, 0, testKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
  ...
  @Test(
      enabled = true,
      description = "Query dynamic energy chain parameters",
      groups = {"daily"})
  public void test01QueryDynamicEnergyParams() {
    long dynamicEnergyThreshold =
        PublicMethod.getChainParametersValue("getDynamicEnergyThreshold", blockingStubFull);
    long dynamicEnergyIncreaseFactor =
        PublicMethod.getChainParametersValue("getDynamicEnergyIncreaseFactor", blockingStubFull);
    long dynamicEnergyMaxFactor =
        PublicMethod.getChainParametersValue("getDynamicEnergyMaxFactor", blockingStubFull);
    ...
    Assert.assertTrue(
        dynamicEnergyThreshold >= 0, "Dynamic energy threshold should be non-negative");
    Assert.assertTrue(
        dynamicEnergyMaxFactor >= 0, "Dynamic energy max factor should be non-negative");
  }

  @Test(
      enabled = true,
      description = "Query contract energy usage state",
      groups = {"daily"})
  public void test02QueryContractState() {
    ContractState state =
        PublicMethod.getContractInfo(contractAddress, blockingStubFull).getContractState();
    ...
    Assert.assertTrue(state.getEnergyFactor() >= 0, "Energy factor should be non-negative");
  }

  @Test(
      enabled = true,
      description = "Trigger contract and verify energy consumption recorded",
      groups = {"daily"})
