 ...
 *
 * <p>Validates that blocks are produced at the expected interval (~3s), by valid witnesses, and
 * that block numbers increment monotonically.
 */
@Slf4j
public class BlockProductionTest extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Blocks should have incrementing block numbers",
      groups = {"daily"})
  ...
  @Test(
      enabled = true,
      description = "Block producer should be a valid witness",
      groups = {"daily"})
  public void test02BlockProducerIsValidWitness() {
    ...
    Assert.assertTrue(
        validWitnessAddresses.contains(producerAddr),
        "Block producer should be a registered witness");
  }

  @Test(
      enabled = true,
      description = "Block timestamps should be ~3 seconds apart",
      groups = {"daily"})
  public void test03BlockTimestampInterval() {
    ...
    Assert.assertTrue(
        intervalMs >= 1000 && intervalMs <= 12000,
        "Block interval should be roughly 3 seconds, got " + intervalMs + " ms");
  }

  @Test(
      enabled = true,
      description = "Multiple consecutive blocks produced by different witnesses",
      groups = {"daily"})
  public void test04WitnessRotation() {
    Set<String> producers = new HashSet<>();
    long currentNum =
        blockingStubFull
            .getNowBlock(org.tron.api.GrpcAPI.EmptyMessage.newBuilder().build())
            .getBlockHeader()
            .getRawData()
            .getNumber();
    ...
    Assert.assertTrue(producers.size() >= 1, "Should have at least 1 distinct block producer");
  }
}
