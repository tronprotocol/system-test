...
public class WalletTestAssetIssue015 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Use transfer net when token owner has not enough bandwidth",
      groups = {"daily"})
  public void atestWhenCreatorHasNoEnoughBandwidthUseTransferNet() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            asset015Address, sendAmount, foundationAddress, foundationKey, blockingStubFull));
    ...
    AccountNetMessage assetCreatorNet =
        PublicMethod.getAccountNet(asset015Address, blockingStubFull);
    ...
    Assert.assertTrue(
        PublicMethod.transferAsset(
            ...
            blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    assetCreatorNet = PublicMethod.getAccountNet(asset015Address, blockingStubFull);
  ...
  @Test(
      enabled = true,
      description = "Use balance when transfer has not enough net",
      groups = {"daily"})
  public void btestWhenTransferHasNoEnoughBandwidthUseBalance() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            transferAssetAddress, 20000000, foundationAddress, foundationKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
  ...
  @Test(
      enabled = true,
      description = "Transfer asset use bandwidth when freeze balance",
      groups = {"daily"})
  public void ctestWhenFreezeBalanceUseNet() {
    Assert.assertTrue(
        PublicMethod.freezeBalance(
            transferAssetAddress, 5000000, 3, transferAssetCreateKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    AccountNetMessage assetTransferNet =
        PublicMethod.getAccountNet(transferAssetAddress, blockingStubFull);
    Account transferAccount = PublicMethod.queryAccount(transferAssetCreateKey, blockingStubFull);
  ...
  public void shutdown() throws InterruptedException {
    PublicMethod.freeResource(
        asset015Address, testKeyForAssetIssue015, foundationAddress, blockingStubFull);
    PublicMethod.freeResource(
        transferAssetAddress, transferAssetCreateKey, foundationAddress, blockingStubFull);
  }
}
