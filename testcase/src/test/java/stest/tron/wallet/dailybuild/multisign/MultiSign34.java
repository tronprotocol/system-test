...
public class MultiSign34 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "SR witness,sendcoin, use witnessPermission address sign.",
      groups = {"daily", "multisig"})
  public void testMultiUpdatepermissions_42() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            witnessAddress2, 1000000000, foundationAddress, testKey002, blockingStubFull));
    ...
    Transaction transaction =
        PublicMethodForMultiSign.sendcoinWithPermissionIdNotSign(
            foundationAddress, 1L, witnessAddress2, 1, witnessKey2, blockingStubFull);
    Transaction transaction1 =
        PublicMethod.addTransactionSign(transaction, witnessKey2, blockingStubFull);
    final TransactionSignWeight transactionSignWeight =
    ...
    Assert.assertThat(
        returnResult1.getMessage().toStringUtf8(), containsString("permission isn't exit"));
  }

  @Test(
      enabled = true,
      description = "SR witness,sendcoin, use active address sign.",
      groups = {"daily", "multisig"})
  public void testMultiUpdatepermissions_43() {
    ...
    Transaction transaction =
        PublicMethodForMultiSign.sendcoinWithPermissionIdNotSign(
            foundationAddress, 1L, witnessAddress2, 2, witnessKey2, blockingStubFull);
  ...
  @Test(
      enabled = true,
      description = "SR witness,sendcoin, use owner address sign.",
      groups = {"daily", "multisig"})
  public void testMultiUpdatepermissions_44() {
    ...
    Transaction transaction =
        PublicMethodForMultiSign.sendcoinWithPermissionIdNotSign(
            foundationAddress, 1L, witnessAddress2, 0, witnessKey2, blockingStubFull);
    final Transaction transaction1 =
        PublicMethod.addTransactionSign(transaction, witnessKey2, blockingStubFull);
    Return returnResult1 =
        PublicMethodForMultiSign.broadcastTransaction1(transaction1, blockingStubFull);
