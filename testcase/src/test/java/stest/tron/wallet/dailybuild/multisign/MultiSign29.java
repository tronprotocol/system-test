...
@Slf4j
public class MultiSign29 extends TronBaseTest {  private final String testKey003 = Configuration.getByPath("testng.conf")
  ...
  @Test(enabled = true, description = "Sendcoin,use active address sign, not meet the requirements."
      + "Then use permissionID same in active  address to sign,broadcastTransaction.", groups = {"daily", "multisig"})
  public void testMultiUpdatepermissions_23() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(test001Address, amount, foundationAddress, foundationKey, blockingStubFull));
  ...
  @Test(enabled = true, description = "Sendcoin with permission id 0,use owner address sign,"
      + "Then use owner address to sign, sum weight > threshold,broadcastTransaction.", groups = {"daily", "multisig"})
  ...
  String accountPermissionJson1 = "{\"owner_permission\":{\"type\":0,\"permission_name\":"
        + "\"owner\",\"threshold\":1,\"keys\":[{\"address\":" + "\"" + PublicMethod
        .getAddressString(dev001Key) + "\",\"weight\":1}," + "{\"address\":\"" + PublicMethod
  ...
  @Test(enabled = true, description = "Sendcoin,use active address sign, not meet the requirements."
      + "Then use owner address to sign, not meet the requirements,broadcastTransaction.", groups = {"daily", "multisig"})
  ...
  @Test(enabled = true, description = "Sendcoin,use owner address sign,  meet all requirements."
      + "Then use active address to sign no permission id,broadcastTransaction.", groups = {"daily", "multisig"})
  ...
  @Test(enabled = true, description = "Sendcoin,use owner address sign,  meet all requirements."
      + "Then use active address to sign with permission id 2,broadcastTransaction.", groups = {"daily", "multisig"})
