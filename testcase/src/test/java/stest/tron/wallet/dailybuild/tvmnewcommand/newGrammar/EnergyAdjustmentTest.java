...
public class EnergyAdjustmentTest extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethod.getContract(contractAddressC, blockingStubFull);
  ...
  @Test(
      ...
      description =
          "before committe no.81,memory extend does not cost fee,"
              + " offset too big will result in cpu time out. after committe no.81, offset too big "
  ...
  @Test(
      enabled = true,
      description = "call sucide twice to active the same account",
      groups = {"contract", "daily"})
  ...
  void suicideToActiveAccount04() {
    ...
    long balance =
        PublicMethod.queryAccount(ByteArray.fromHexString(sub), blockingStubFull).getBalance();
    Assert.assertEquals(trxValue, balance);
  }

  @Test(
      enabled = true,
      description = "create2 address sucide to active one account",
      groups = {"contract", "daily"})
