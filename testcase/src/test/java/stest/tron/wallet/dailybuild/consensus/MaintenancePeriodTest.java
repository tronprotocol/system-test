 ...
 *
 * <p>Validates that the maintenance period mechanism is active and that the next maintenance time
 * is in the future relative to the latest block.
 */
@Slf4j
public class MaintenancePeriodTest extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Next maintenance time should be in the future",
      groups = {"daily"})
  public void test01NextMaintenanceTimeInFuture() {
    NumberMessage nextMaintenance =
    ...
    Assert.assertTrue(nextMaintenanceMs > 0, "Next maintenance time should be positive");
    Assert.assertTrue(
        nextMaintenanceMs >= blockTimestamp,
        "Next maintenance time should be >= current block timestamp");
  }

  @Test(
      enabled = true,
      description = "Witness list should be available and non-empty after maintenance",
      groups = {"daily"})
  ...
  @Test(
      enabled = true,
      description = "Total vote count across all witnesses should be positive",
      groups = {"daily"})
  public void test03TotalVoteCountPositive() {
    Optional<WitnessList> witnessList = PublicMethod.listWitnesses(blockingStubFull);
    Assert.assertTrue(witnessList.isPresent());
    long totalVotes =
        witnessList.get().getWitnessesList().stream().mapToLong(w -> w.getVoteCount()).sum();
