...
public class HttpTestSmartContract001 {
  ...
  public void test1DeployContract() {
    ...
    response = HttpMethod.sendCoin(httpnode, fromAddress, assetReceiverAddress, amount, testKey002);
    Assert.assertTrue(HttpMethod.verificationResult(response));
    HttpMethod.waitToProduceOneBlock(httpnode);
    // Create an asset issue
    response =
        HttpMethod.freezeBalance(httpnode, assetOwnerAddress, 100000000L, 3, 1, assetOwnerKey);
    Assert.assertTrue(HttpMethod.verificationResult(response));
    response =
    ...
    /*response = HttpMethod.deployContractGetTxidWithTooBigLong(httpnode,
    ...
    Assert.assertEquals(
        HttpMethod.parseStringContent(receiptString).getString("result"), "SUCCESS");
  }
  ...
  public void test3TriggerContract() {
    ...
    final Long beforeBalance = HttpMethod.getBalance(httpnode, assetOwnerAddress);
    ...
    response = HttpMethod.getAccount(httpnode, assetReceiverAddress);
    responseContent = HttpMethod.parseResponseContent(response);
    HttpMethod.printJsonContent(responseContent);
    ...
    for (; times < 10; times++) {
      ...
      txid2 =
          HttpMethod.triggerContractGetTxid(
              ...
              assetOwnerKey);
      HttpMethod.waitToProduceOneBlock(httpnode);
      HttpResponse response1 = HttpMethod.getTransactionInfoById(httpnode, txid1);
      HttpResponse response2 = HttpMethod.getTransactionInfoById(httpnode, txid2);
      responseCon1 = HttpMethod.parseResponseContent(response1);
      HttpMethod.printJsonContent(responseCon1);
      responseCon2 = HttpMethod.parseResponseContent(response2);
  ...
  public void test7UpdateSetting() {
    ...
    response =
        HttpMethod.updateSetting(httpnode, assetOwnerAddress, contractAddress, 75, assetOwnerKey);
    Assert.assertTrue(HttpMethod.verificationResult(response));
    HttpMethod.waitToProduceOneBlock(httpnode);
    response = HttpMethod.getContract(httpnode, contractAddress);
    responseContent = HttpMethod.parseResponseContent(response);
  ...
  public void shutdown() throws InterruptedException {
    ...
    HttpMethod.freeResource(httpnode, assetReceiverAddress, fromAddress, assetReceiverKey);
