...
public class TransactionPendingQuery001 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Test get pending size",
      groups = {"daily"})
  public void test01GetPendingSize() {
    ...
    while (pendingSizeInFullNode == 0 && retryTimes-- > 0) {
      ...
      if (retryTimes % 5 == 0) {
        pendingSizeInFullNode =
            blockingStubFull.getPendingSize(EmptyMessage.newBuilder().build()).getNum();
      }
    }
  ...
  @Test(
      enabled = true,
      description = "Test get pending transaction list",
      groups = {"daily"})
  public void test02GetPendingTransactionList() {
    int retryTimes = 100;
    TransactionIdList transactionList =
        blockingStubFull.getTransactionListFromPending(EmptyMessage.newBuilder().build());
  ...
  @Test(
      enabled = true,
      description = "Test transaction from pending",
      groups = {"daily"})
  public void test03GetTransactionFromPending() {
    ...
    Assert.assertEquals(
        ByteArray.toHexString(Sha256Hash.hash(true, transaction.getRawData().toByteArray())), txid);
  }
  ...
  public void shutdown() throws InterruptedException {
    PublicMethod.unFreezeBalance(
        receiverAddress, receiverKey, 1, receiverAddress, blockingStubFull);
