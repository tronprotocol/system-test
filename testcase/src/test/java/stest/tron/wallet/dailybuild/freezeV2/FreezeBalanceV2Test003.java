...
@Slf4j
@Flaky(reason = "Timing-sensitive: depends on maintenance interval", since = "2026-04-03")
public class FreezeBalanceV2Test003 extends TronBaseTest {
  ...
  public void beforeClass() throws Exception {
    ...
    Assert.assertTrue(
        PublicMethod.getTransactionById(txid, blockingStubFull).get().getSignatureCount() == 1);
  }

  @Test(
      enabled = true,
      description = "MultiSign for freeze balance V2",
      groups = {"daily", "staking"})
  public void test01MultiSignForFreezeBalanceV2() {
    Assert.assertTrue(
        PublicMethodForMultiSign.freezeBalanceV2WithPermissionId(
            ownerAddress, freezeBalance, 0, 2, ownerKey, blockingStubFull, permissionKeyString));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    Transaction transaction =
        PublicMethod.getTransactionById(PublicMethodForMultiSign.freezeV2Txid, blockingStubFull)
            .get();
  ...
  @Test(
      enabled = true,
      description = "MultiSign for delegate resource",
      groups = {"daily", "staking"})
  public void test02MultiSignForDelegateResource() {
    Assert.assertTrue(
        PublicMethodForMultiSign.delegateResourceWithPermissionId(
            ownerAddress,
            delegateBalance,
            0,
            receiverAddress,
  ...
  @Test(
      enabled = true,
      description = "MultiSign for release delegate resource",
      groups = {"daily", "staking"})
  public void test03MultiSignForUnDelegateResource() {
    Assert.assertTrue(
        PublicMethodForMultiSign.unDelegateResourceWithPermissionId(
            ownerAddress,
            delegateBalance,
            0,
            receiverAddress,
  ...
  @Test(
      enabled = true,
      description = "MultiSign for unFreezeBalanceV2",
      groups = {"daily", "staking"})
  public void test04MultiSignForUnFreezeBalanceV2() {
    Assert.assertTrue(
        PublicMethodForMultiSign.unFreezeBalanceV2WithPermissionId(
  ...
  @Test(
      enabled = true,
      description = "MultiSign for withdrawExpireUnfreezeBalance",
      groups = {"daily", "staking"})
  public void test05WithdrawExpireUnfreezeBalance() {
    ...
    if (System.currentTimeMillis() - 2000L < account.getUnfrozenV2(0).getUnfreezeExpireTime()) {
      Assert.assertFalse(
          PublicMethodForMultiSign.withdrawExpireUnfreezeBalanceWithPermissionId(
  ...
  @Test(
      enabled = true,
      description = "MultiSign for cancelUnfreeze",
      groups = {"daily", "staking"})
  public void test06MultiSignForCancelUnfreeze() {
    ...
    li.add(0);
    Assert.assertTrue(
        PublicMethodForMultiSign.cancelAllUnfreezeWithPermissionId(
            ownerAddress, 2, blockingStubFull, permissionKeyString));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    TransactionInfo info =
        PublicMethod.getTransactionInfoById(
                PublicMethodForMultiSign.cancelUnfreezeTxId, blockingStubFull)
            .get();
