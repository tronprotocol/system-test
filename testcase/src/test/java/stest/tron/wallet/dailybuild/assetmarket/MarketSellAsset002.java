...
public class MarketSellAsset002 extends TronBaseTest {
  ...
  @Test(
      enabled = false,
      description = "create sellOrder and Match Order",
      groups = {"daily"})
  void marketSellAssetTest001() {

    Map<String, Long> beforeAsset001 =
        PublicMethod.queryAccount(testAddress001, blockingStubFull).getAssetV2Map();
    Map<String, Long> beforeAsset002 =
    ...
    transaction = PublicMethod.getTransactionById(txid2, blockingStubFull);
    Assert.assertEquals(transaction.get().getRet(0).getRet(), code.SUCESS);

    Map<String, Long> afterAsset001 =
        PublicMethod.queryAccount(testAddress001, blockingStubFull).getAssetV2Map();
    Map<String, Long> afterAsset002 =
        PublicMethod.queryAccount(testAddress002, blockingStubFull).getAssetV2Map();
  ...
  @Test(
      enabled = false,
      description = "create sellOrder and Match Order twice",
      groups = {"daily"})
  void marketSellAssetTest002() {
    ...
    MarketOrder order001 = PublicMethod.getMarketOrderById(orderId, blockingStubFull).get();
    ...
    String assetId002 = ByteArray.toStr(assetAccountId002);
    Assert.assertEquals(
        (beforeAsset001.get(assetId001) - sellTokenQuantity * 2),
        afterAsset001.get(assetId001).longValue());
    Assert.assertEquals(
        (beforeAsset001.get(assetId002) + buyTokenQuantity),
    ...
    Assert.assertEquals(
        ...
        afterAsset002.get(assetId002).longValue());
    Assert.assertEquals(
        beforeAsset002.get(assetId001) + sellTokenQuantity,
    ...
    transaction = PublicMethod.getTransactionById(txid3, blockingStubFull);
    Assert.assertEquals(transaction.get().getRet(0).getRet(), code.SUCESS);
    // get order Message and RemainSellTokenQuantity
    order001 = PublicMethod.getMarketOrderById(orderId, blockingStubFull).get();
    Assert.assertEquals(order001.getSellTokenQuantityRemain(), 0);

    afterAsset001 = PublicMethod.queryAccount(testAddress001, blockingStubFull).getAssetV2Map();
    afterAsset002 = PublicMethod.queryAccount(testAddress002, blockingStubFull).getAssetV2Map();
    ...
    Assert.assertEquals(
        ...
        afterAsset001.get(assetId001).longValue());
    Assert.assertEquals(
        (beforeAsset001.get(assetId002) + buyTokenQuantity * 2),
        afterAsset001.get(assetId002).longValue());

    Assert.assertEquals(
        beforeAsset002.get(assetId002) - buyTokenQuantity * 2,
        afterAsset002.get(assetId002).longValue());
    Assert.assertEquals(
        beforeAsset002.get(assetId001) + sellTokenQuantity * 2,
        afterAsset002.get(assetId001).longValue());
  }

  @Test(
      enabled = false,
      description = "create sellOrder and not Match Order",
      groups = {"daily"})
  void marketSellAssetTest003() {
    ...
    Assert.assertEquals(
        ...
        afterAsset001.get(assetId001).longValue());
    Assert.assertEquals(
        (beforeAsset001.get(assetId002).longValue()), afterAsset001.get(assetId002).longValue());

    Assert.assertEquals(
        beforeAsset002.get(assetId002) - buyTokenQuantity * 2,
        afterAsset002.get(assetId002).longValue());
    Assert.assertEquals(
        beforeAsset002.get(assetId001).longValue(), afterAsset002.get(assetId001).longValue());
  }
  ...
  void marketSellAssetTest004() {
    ...
    String txid =
        PublicMethod.marketCancelOrder(testAddress001, testKey001, orderId001, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    Assert.assertNotNull(txid);
    ...
    Assert.assertEquals(
        beforeAsset001.get(assetId001) + sellTokenQuantity001,
        afterAsset001.get(assetId001).longValue());

    Return response =
        PublicMethod.marketCancelOrderGetResposne(
            testAddress001, testKey001, orderId001, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    Assert.assertNotNull(response);
    Assert.assertEquals(response.getCode(), response_code.CONTRACT_VALIDATE_ERROR);
    Assert.assertEquals(
        ByteArray.toStr(response.getMessage().toByteArray()).toLowerCase(),
