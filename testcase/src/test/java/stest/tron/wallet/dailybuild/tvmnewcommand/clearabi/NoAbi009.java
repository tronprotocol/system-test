...
@Slf4j
@MultiNode
public class NoAbi009 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "clearabi contract with event",
  ...
  public void testNoAbi001() {
    ...
    SmartContractDataWrapper contractInfo =
        PublicMethod.getContractInfo(contractAddress, blockingStubFull);
    ...
    List<String> retList =
        PublicMethod.getStrings(infoById.get().getLogList().get(0).getData().toByteArray());
  ...
  @Test(
      enabled = true,
      description = "create2 contract with event",
      groups = {"contract", "daily"})
  public void testNoAbi002() {
    ...
    // getcontract
    smartContract = PublicMethod.getContract(testNoABiContractAddress, blockingStubFull);
    System.out.println("smartContract:" + smartContract.toString());
    ...
    // getcontractinfo
    SmartContractDataWrapper contractInfo =
        PublicMethod.getContractInfo(testNoABiContractAddress, blockingStubFull);
    System.out.println("contractInfo.toString():" + contractInfo.toString());
    Assert.assertTrue(contractInfo.getSmartContract().getAbi().toString().isEmpty());
    // Assert.assertTrue(contractInfo.getSmartContract().getName().equalsIgnoreCase(contractName));
    Assert.assertTrue(contractInfo.getRuntimecode().size() > 0);
    ...
    retList = PublicMethod.getStrings(infoById.get().getLogList().get(0).getData().toByteArray());
    logger.info("retList:" + retList.toString());
