...
public class TvmAssetIssue004 extends TronBaseTest {
  ...
  @Test(
      enabled = false,
      description = "tokenIssue and transfer to account",
  ...
  public void tokenIssueAndTransferToAccount() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            dev003Address, 10_000_000L, dev001Address, dev001Key, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    long dev001AddressBalanceBefore =
    ...
    logger.info(infoById.toString());
    Assert.assertEquals(0, infoById.get().getResultValue());
    long dev001AddressBalanceAfter =
        PublicMethod.queryAccount(dev001Address, blockingStubFull).getBalance();
    ...
    long dev001AddressBalanceBefore1 =
        PublicMethod.queryAccount(dev001Address, blockingStubFull).getBalance();
