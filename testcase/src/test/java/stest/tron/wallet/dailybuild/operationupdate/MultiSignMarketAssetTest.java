...
@Slf4j
public class MultiSignMarketAssetTest extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethod.createAssetIssue(
            ...
            testKey001,
            blockingStubFull));
    Long balanceBefore = PublicMethod.queryAccount(testAddress001, blockingStubFull).getBalance();
    ...
    accountPermissionJson =
        "{\"owner_permission\":{\"type\":0,\"permission_name\":\"owner\",\"threshold\":2,\"keys\":["
            + "{\"address\":\""
            + PublicMethod.getAddressString(testKey001)
            + "\",\"weight\":1},"
            + "{\"address\":\""
  ...
  @Test(
      enabled = false,
      description = "MultiSignForMarketSellAsset with active_permissions",
  ...
  public void testMultiSignForMarketSellAsset001() {
    ...
    Assert.assertTrue(
        PublicMethodForMultiSign.marketSellAsset(
            testAddress001,
    ...
    MarketOrderList marketOrder =
        PublicMethod.getMarketOrderByAccount(testAddress001, blockingStubFull).get();
    Assert.assertEquals(marketOrderCountBefore + 1, marketOrder.getOrdersCount());
  }

  @Test(
      enabled = false,
      description = "MultiSignForMarketSellAsset with owner_permission",
      groups = {"contract", "daily"})
  ...
  @Test(
      enabled = false,
      dependsOnMethods = "testMultiSignForMarketSellAsset001",
  ...
  public void testMultiSignForMarketOrderCancel001() {
    ...
    Assert.assertTrue(
        PublicMethodForMultiSign.marketCancelOrder(
  ...
  @Test(
      enabled = false,
      dependsOnMethods = "testMultiSignForMarketSellAsset002",
      description = "MultiSignForMarketOrderCancel with owner_permission",
      groups = {"contract", "daily"})
