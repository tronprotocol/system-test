...
public class WalletTestAssetIssue016 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Get asset issue net resource",
      groups = {"daily"})
  public void test01GetAssetIssueNet() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            asset016Address, sendAmount, foundationAddress2, foundationKey2, blockingStubFull));
    ...
    AccountNetMessage assetIssueInfo =
        PublicMethod.getAccountNet(asset016Address, blockingStubFull);
    ...
    Assert.assertTrue(
        PublicMethod.freezeBalance(
            asset016Address, 30000000L, 3, testKeyForAssetIssue016, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    PublicMethod.transferAsset(
        toAddress,
        assetAccountId.toByteArray(),
    ...
    PublicMethod.freeResource(
        asset016Address, testKeyForAssetIssue016, fromAddress, blockingStubFull);
  }

  @Test(
      enabled = true,
      description = "Get asset issue by name from Solidity",
      groups = {"daily"})
  public void test02GetAssetIssueByNameFromSolidity() {
    ...
    Assert.assertEquals(
        PublicMethod.getAssetIssueByNameFromSolidity(name, blockingStubSolidity).getTotalSupply(),
        totalSupply);
  }

  @Test(
      enabled = true,
      description = "Get asset issue by name from PBFT",
      groups = {"daily"})
  public void test03GetAssetIssueByNameFromPbft() {
    Assert.assertEquals(
        PublicMethod.getAssetIssueByNameFromSolidity(name, blockingStubPbft).getTotalSupply(),
        totalSupply);
  }

  @Test(
      enabled = true,
      description = "Get asset issue list from PBFT",
      groups = {"daily"})
  public void test04GetAssetIssueListFromPbft() {
    Assert.assertTrue(
        PublicMethod.listAssetIssueFromSolidity(blockingStubPbft).get().getAssetIssueCount() >= 1);
  }

  @Test(
      enabled = true,
      description = "Get asset issue list from Solidity",
      groups = {"daily"})
  public void test05GetAssetIssueListFromSolidity() {
    Assert.assertTrue(
        PublicMethod.listAssetIssueFromSolidity(blockingStubSoliInFull).get().getAssetIssueCount()
            >= 1);
    Assert.assertTrue(
        PublicMethod.listAssetIssueFromSolidity(blockingStubSolidity).get().getAssetIssueCount()
            >= 1);
  }

  @Test(
      enabled = true,
      description = "Get asset issue list paginated from PBFT",
      groups = {"daily"})
  public void test06GetAssetIssetListPaginatedFromPbft() {
    Assert.assertTrue(
        PublicMethod.listAssetIssuepaginatedFromSolidity(blockingStubPbft, 0L, 1L)
                .get()
                .getAssetIssueCount()
            == 1);
  }
  ...
  @Test(
      enabled = true,
      description = "Get asset issue list paginated from Solidity",
      groups = {"daily"})
  public void test05GetAssetIssueListPaginatedFromSolidity() {
    Assert.assertTrue(
        PublicMethod.listAssetIssuepaginatedFromSolidity(blockingStubSolidity, 0L, 1L)
                .get()
                .getAssetIssueCount()
            == 1);
    Assert.assertTrue(
        PublicMethod.listAssetIssuepaginatedFromSolidity(blockingStubSoliInFull, 0L, 1L)
                .get()
                .getAssetIssueCount()
            == 1);
  }
