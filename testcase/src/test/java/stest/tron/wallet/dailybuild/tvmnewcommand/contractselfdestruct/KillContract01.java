...
@Slf4j
@Flaky(reason = "Selfdestruct timing between blocks", since = "2026-04-03")
public class KillContract01 extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethod.getContract(contractAddressD, blockingStubFull);
  ...
  @Test(
      ...
      description =
          "getPredictedAddress,sendcoin,transferAsset,and then "
              + "make create2,freezev2,delegateResource,delegateResource,unfreezeBalanceV2,cancelAl"
              + "lUnfreezeV2,vote,kill "
              + "into one tranaction",
  ...
  void kill01() {
    ...
    Account delegateReceiverAccount =
        PublicMethod.queryAccount(delegateReceiverAdd, blockingStubFull);
    ...
    long targetAssetCount =
        PublicMethod.getAssetIssueValue(targetAdd, assetAccountId, blockingStubFull);
    ...
    AccountResourceMessage delegateReceiverResource =
        PublicMethod.getAccountResource(delegateReceiverAdd, blockingStubFull);
    ...
    AccountResourceMessage targetResource =
        PublicMethod.getAccountResource(targetAdd, blockingStubFull);
    ...
    long delegateReceiverAssetCount =
        PublicMethod.getAssetIssueValue(delegateReceiverAdd, assetAccountId, blockingStubFull);
    Assert.assertEquals(0, delegateReceiverAssetCount);
    long execAssetCount = PublicMethod.getAssetIssueValue(excAdd, assetAccountId, blockingStubFull);
  ...
  @Test(
      ...
      description =
          "getPredictedAddress,sendcoin,transferAsset,and then "
              + "make create2,freezev2,delegateResource,delegateResource,unfreezeBalanceV2,cancelAl"
              + "lUnfreezeV2,"
              + "vote into one transaction, and kill in another transaction. and can not static cal"
              + "l kill",
      groups = {"contract", "daily"})
  void kill02() {
    String methedStr = "getPredictedAddress(bytes32)";
  ...
  void kill03() {
    ...
    Assert.assertEquals(0, targetAccount.getVotesList().size());
    long targetAssetCount =
        PublicMethod.getAssetIssueValue(targetAdd03, assetAccountId, blockingStubFull);
    Assert.assertEquals(100, targetAssetCount);
    AccountResourceMessage targetResource =
        PublicMethod.getAccountResource(targetAdd03, blockingStubFull);
    Assert.assertNotEquals(0, targetResource.getNetLimit());
    Assert.assertNotEquals(0, targetResource.getEnergyLimit());
  }
}
