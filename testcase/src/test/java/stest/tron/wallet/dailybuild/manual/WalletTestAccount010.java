...
public class WalletTestAccount010 extends TronBaseTest {
  ...
  public void testGetStorage() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account010Address, 100000000, foundationAddress, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account010SecondAddress,
            100000000,
            foundationAddress,
            foundationKey,
            blockingStubFull));
    ...
    Assert.assertTrue(
        PublicMethod.buyStorage(100000000L, account010Address, account010Key, blockingStubFull));

    account010Info = PublicMethod.queryAccount(account010Key, blockingStubFull);
    Assert.assertTrue(account010Info.getAccountResource().getStorageLimit() > 0);
    Assert.assertTrue(account010Info.getAccountResource().getLatestExchangeStorageTime() > 0);

    AccountResourceMessage account010Resource =
        PublicMethod.getAccountResource(account010Address, blockingStubFull);
  ...
  public void testSellStorage() {
    ...
    Assert.assertFalse(
        PublicMethod.sellStorage(
            storageLimit + 1, account010Address, account010Key, blockingStubFull));
    // Can not sell 0 storage
    Assert.assertFalse(
        PublicMethod.sellStorage(0, account010Address, account010Key, blockingStubFull));
    // Sell all storage.
    Assert.assertTrue(
        PublicMethod.sellStorage(storageLimit, account010Address, account010Key, blockingStubFull));
    account010Resource = PublicMethod.getAccountResource(account010Address, blockingStubFull);
