...
@Slf4j
public class ContractTrcToken001 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "DeployContract with correct tokenValue and tokenId",
      groups = {"contract", "daily"})
  public void deployTransferTokenContract() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            dev001Address, 5100_000_000L, fromAddress, foundationKey, blockingStubFull));

    PublicMethod.waitProduceNextBlock(blockingStubFull);
    //    Assert.assertTrue(PublicMethod.freezeBalanceForReceiver(fromAddress,
    //        PublicMethod.getFreezeBalanceCount(dev001Address, dev001Key, 135678L,
    // blockingStubFull), 0,
    //        1, ByteString.copyFrom(dev001Address), foundationKey, blockingStubFull));
    //
    //    Assert.assertTrue(PublicMethod.freezeBalanceForReceiver(fromAddress, 10_000_000L, 0, 0,
    ...
    List<String> retList =
        PublicMethod.getStrings(infoById.get().getContractResult(0).toByteArray());
