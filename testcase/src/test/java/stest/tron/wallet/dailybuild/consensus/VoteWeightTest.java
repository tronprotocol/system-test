 ...
 *
 * <p>Verifies that voting power is proportional to frozen TRX (Tron Power), and that votes can be
 * updated and revoked.
 */
@Slf4j
public class VoteWeightTest extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Freeze more TRX = more vote weight (Stake 2.0)",
      groups = {"daily"})
  public void test01MoreFreezeMoreVotes() {
    ...
    long power1 = acc1.getFrozenV2List().stream().mapToLong(Account.FreezeV2::getAmount).sum();
    long power2 = acc2.getFrozenV2List().stream().mapToLong(Account.FreezeV2::getAmount).sum();
    logger.info("Voter1 frozenV2: {}, Voter2 frozenV2: {}", power1, power2);
    Assert.assertTrue(
        power1 > power2, "Voter1 (10000 TRX) should have more tron power than Voter2 (5000 TRX)");
  }

  @Test(
      enabled = true,
      dependsOnMethods = "test01MoreFreezeMoreVotes",
  ...
  public void test02VoteCountMatchesFrozenAmount() {
    ...
    Assert.assertTrue(
        PublicMethod.voteWitness(voter1Address, voter1KeyStr, voteMap1, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);

    Account acc1 = PublicMethod.queryAccount(voter1Address, blockingStubFull);
    Assert.assertTrue(acc1.getVotesCount() > 0);
    Assert.assertEquals(
        acc1.getVotes(0).getVoteCount(),
        voter1Amount,
  ...
  @Test(
      enabled = true,
      dependsOnMethods = "test02VoteCountMatchesFrozenAmount",
  ...
  public void test03ReVoteUpdatesCount() {
    ...
    Assert.assertTrue(
        PublicMethod.voteWitness(voter1Address, voter1KeyStr, voteMap, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);

    Account acc = PublicMethod.queryAccount(voter1Address, blockingStubFull);
    Assert.assertEquals(
        acc.getVotes(0).getVoteCount(), newAmount, "Re-voting should update vote count");
  }

  @Test(
      enabled = true,
      dependsOnMethods = "test03ReVoteUpdatesCount",
