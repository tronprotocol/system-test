...
@Slf4j
public class MultiSign20 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Owner address is witness",
  ...
  @Test(
      enabled = true,
      description = "Owner address is witness with exception condition",
      groups = {"daily", "multisig"})
  public void testOwnerAddress02() {
    ...
    Assert.assertEquals(
        "Contract validate error : witness permission is missed",
    ...
    Assert.assertEquals(CONTRACT_VALIDATE_ERROR, response.getCode());
    Assert.assertEquals(
        "Contract validate error : owner permission is missed",
        response.getMessage().toStringUtf8());
    Long balanceAfter = PublicMethod.queryAccount(ownerAddress, blockingStubFull).getBalance();
    logger.info("balanceAfter: " + balanceAfter);
  ...
  @Test(
      enabled = true,
      description = "Owner address is normal address with exception condition",
      groups = {"daily", "multisig"})
  public void testOwnerAddress03() {
    ...
    Assert.assertEquals(CONTRACT_VALIDATE_ERROR, response.getCode());
    Assert.assertEquals(
        "Contract validate error : invalidate ownerAddress", response.getMessage().toStringUtf8());
    ...
    response =
        PublicMethod.accountPermissionUpdateForResponse(
            accountPermissionJson, ownerAddressTmp, ownerKeyTmp, blockingStubFull);
    Assert.assertFalse(response.getResult());
    Assert.assertEquals(CONTRACT_VALIDATE_ERROR, response.getCode());
    Assert.assertEquals(
        "Contract validate error : ownerAddress account does not exist",
        response.getMessage().toStringUtf8());
    // address = not exist
  ...
  public void aftertest() {
    PublicMethod.unFreezeBalance(
        foundationAddress, foundationKey, 0, ownerAddress, blockingStubFull);
    PublicMethod.unFreezeBalance(
        foundationAddress, foundationKey, 1, ownerAddress, blockingStubFull);
