...
@Slf4j
public class NewFeatureForSolc0820 extends TronBaseTest {
  ...
  String abi = retMap.get("abI").toString();
    contract1 = PublicMethod.deployContract(contractName, abi, code, "", maxFeeLimit,
...
//    PublicMethod.waitProduceNextBlock(blockingStubFull);
    ...
    contract2 = PublicMethod.deployContract(contractName, abi, code, "", maxFeeLimit,
        0L, 100, null, contractExcKey,
        contractExcAddress, blockingStubFull);
    ...
    code = retMap.get("byteCode").toString();
    abi = retMap.get("abI").toString();
    lib = PublicMethod.deployContract(contractName, abi, code, "", maxFeeLimit,
        0L, 100, null, contractExcKey,
        contractExcAddress, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
  ...
  String compilerVersion = Configuration.getByPath("testng.conf")
    ...
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    SmartContractOuterClass.SmartContract smartContract = PublicMethod.getContract(contract1,
        blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());

    smartContract = PublicMethod.getContract(contract2, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());

    smartContract = PublicMethod.getContract(contract3, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());
  }

  @Test(enabled = true, description = "test using for T global inside sol", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test using for T global beside sol", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test external library functions in ``using for``", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test public library functions in ``using for`` ", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test internal library functions in ``using for`` ", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "emit event by a contract but defined outside of it.", groups = {"contract", "daily"})
