...
public class FreezeBalanceV2Test004 {
  ...
  public void test01MaxUnfreezeBalanceListIs32() throws Exception {
    ...
    while (unfreezeTimes++ <= 50) {
      PublicMethed.unFreezeBalanceV2(frozenBandwidthAddress,
          frozenBandwidthKey, unfreezeBalance, 0, blockingStubFull);
      Thread.sleep(100L);
    }
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertTrue(PublicMethed.unFreezeBalanceV2(frozenBandwidthAddress,
        frozenBandwidthKey, unfreezeBalance, 0, blockingStubFull));

    PublicMethed.waitProduceNextBlock(blockingStubFull);

    final TransactionInfo transactionInfo =
        PublicMethed.getTransactionInfoById(PublicMethed.freezeV2Txid, blockingStubFull).get();
    account = PublicMethed.queryAccount(frozenBandwidthAddress, blockingStubFull);
  ...
  public void test02UnfreezeCaseDynamicDecreaseOfVote() throws Exception {
    ...
    Account account = PublicMethed.queryAccount(frozenBandwidthAddress, blockingStubFull);
    final Long votePower = PublicMethed.tronPowerProposalIsOpen(blockingStubFull)
        ?
    ...
    Assert.assertTrue(PublicMethed.voteWitness(frozenBandwidthAddress,
        frozenBandwidthKey, voteMap, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    if (PublicMethed.tronPowerProposalIsOpen(blockingStubFull)) {
      Assert.assertTrue(PublicMethed.freezeBalanceV2(frozenBandwidthAddress,
          ...
          2, frozenBandwidthKey, blockingStubFull));
    } else {
      Assert.assertTrue(PublicMethed.freezeBalanceV2(frozenBandwidthAddress,
          (witness1ReceiveVote + witness2ReceiveVote) * 1000000L,
