...
@Slf4j
@Flaky(reason = "Timing-sensitive: unfreeze wait period race condition", since = "2026-04-03")
public class FreezeBalanceV2Test004 extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Max unfreeze balance list is 32",
  ...
  public void test01MaxUnfreezeBalanceListIs32() throws Exception {
    ...
    // query solidity
    Assert.assertTrue(
        PublicMethod.getAvailableUnfreezeCountSolidity(
    ...
    // query pbft
    Assert.assertTrue(
        PublicMethod.getAvailableUnfreezeCountSolidity(frozenBandwidthAddress, blockingStubPbft)
                .get()
                .getCount()
            == 0);
    Assert.assertTrue(
  ...
  @Test(
      enabled = true,
      description = "Unfreeze cause dynamic decrease of vote",
      groups = {"daily", "staking"})
  public void test02UnfreezeCaseDynamicDecreaseOfVote() throws Exception {
    ...
    Account account = PublicMethod.queryAccount(frozenBandwidthAddress, blockingStubFull);
    Long votePower =
        !PublicMethod.tronPowerProposalIsOpen(blockingStubFull)
    ...
    Assert.assertTrue(
        PublicMethod.unFreezeBalanceV2(
            ...
            frozenBandwidthKey,
            unfreezeBalance,
            PublicMethod.tronPowerProposalIsOpen(blockingStubFull) ? 2 : 0,
            blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
