...
@Slf4j
@Flaky(reason = "Timing-sensitive: depends on maintenance interval and unfreeze wait period",
    since = "2026-04-03")
public class FreezeBalanceV2Test001 extends TronBaseTest {
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Freeze balance to get bandwidth", groups = {"daily", "staking"})
  ...
  String txId = PublicMethod.freezeBalanceV2AndGetTxId(frozenBandwidthAddress,
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Freeze balance to get energy", groups = {"daily", "staking"})
  ...
  String txId = PublicMethod.freezeBalanceV2AndGetTxId(frozenEnergyAddress,
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Unfreeze balance to release bandwidth", groups = {"daily", "staking"})
  ...
  final Long beforeUnfreezeBalance = account.getBalance();
    AccountResourceMessage accountResource = PublicMethod
        .getAccountResource(frozenBandwidthAddress, blockingStubFull);
  final Long beforeTotalNetWeight = accountResource.getTotalNetWeight();
  String txId = PublicMethod.unFreezeBalanceV2AndGetTxId(frozenBandwidthAddress,
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Freeze balance to release energy", groups = {"daily", "staking"})
  public void test04UnFreezeBalanceV2ToReleaseEnergy() throws Exception {
    AccountResourceMessage accountResource = PublicMethod
        .getAccountResource(frozenEnergyAddress, blockingStubFull);
  final Long beforeTotalEnergyWeight = accountResource.getTotalEnergyWeight();
  String txId = PublicMethod.unFreezeBalanceV2AndGetTxId(frozenEnergyAddress,
            frozenEnergyKey, freezeEnergyBalance, 1, blockingStubFull);
    Assert.assertNotNull(txId);
    PublicMethod.waitProduceNextBlock(blockingStubFull);

    Transaction transaction = PublicMethod.getTransactionById(txId,
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Withdraw expire unfreeze to release balance", groups = {"daily", "staking"})
  ...
  final Long energyAccountBeforeBalance = account.getBalance();
    ...
    if (System.currentTimeMillis() - 2000L < account.getUnfrozenV2(0).getUnfreezeExpireTime()) {
      Assert.assertFalse(PublicMethod.withdrawExpireUnfreeze(frozenEnergyAddress,
  ...
  Long unfreezeExpireTime = account.getUnfrozenV2(0).getUnfreezeExpireTime();
    ...
    Long canWithdrawUnFreezeAmount = PublicMethod
  ...
  Long canWithdrawUnFreezeAmountSolidity = PublicMethod
  ...
  Long canWithdrawUnFreezeAmountPbft = PublicMethod
        .getCanWithdrawUnfreezeAmountSolidity(frozenEnergyAddress,
  ...
  String txIdBandwidth = PublicMethod.withdrawExpireUnfreezeAndGetTxId(frozenBandwidthAddress,
            frozenBandwidthKey, blockingStubFull);
  String txIdEnergy = PublicMethod.withdrawExpireUnfreezeAndGetTxId(frozenEnergyAddress,
            frozenEnergyKey, blockingStubFull);
    ...
    Transaction transaction = PublicMethod.getTransactionById(txIdEnergy,
        blockingStubFull).get();
    Any any = transaction.getRawData().getContract(0).getParameter();
  ...
  final Long energyAccountAfterBalance = account.getBalance();
    ...
    TransactionInfo transactionInfo = PublicMethod.getTransactionInfoById(txIdEnergy,
        blockingStubFull).get();
