...
public class MarketSellAsset003 extends TronBaseTest {
  ...
  void marketCancelAssetTest001() {
    ...
    String txid =
        PublicMethod.marketSellAsset(
            testAddress001, testKey001, assetAccountId001, 100, trx, 50, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    ...
    transaction = PublicMethod.getTransactionById(txid, blockingStubFull);
    Assert.assertEquals(transaction.get().getRet(0).getRet().toString(), "SUCESS");

    orderList = PublicMethod.getMarketOrderByAccount(testAddress001, blockingStubFull);
    Assert.assertTrue(orderList.get().getOrdersCount() == 0);
  }

  @Test(
      enabled = false,
      description = "Cancel a cancelled order ",
      groups = {"daily"})
  void marketCancelAssetTest002() {
    ...
    Assert.assertEquals(
        PublicMethod.marketCancelOrder(testAddress001, testKey001, orderId, blockingStubFull)
            .toLowerCase(),
