...
public class MarketSellAsset003 extends TronBaseTest {
  ...
  Long end = System.currentTimeMillis() + 1000000000;
    Assert.assertTrue(PublicMethod.createAssetIssue(testAddress001, name, 10000_000000L,
        1, 1, start, end, 1, description, url, 10000L,
        10000L, 1L, 1L, testKey001, blockingStubFull));

    start = System.currentTimeMillis() + 5000;
    end = System.currentTimeMillis() + 1000000000;
    Assert.assertTrue(PublicMethod.createAssetIssue(testAddress002, name, 10000_000000L,
        1, 1, start, end, 1, description, url, 10000L,
  ...
  byte[] orderId = orderList.get().getOrders(0).getOrderId().toByteArray();
    ...
    orderList = PublicMethod
        .getMarketOrderByAccount(testAddress001, blockingStubFull);
    Assert.assertTrue(orderList.get().getOrdersCount() == 0);

  }

  @Test(enabled = false, description = "Cancel a cancelled order ", groups = {"daily"})
  ...
  byte[] orderId = orderList.get().getOrders(0).getOrderId().toByteArray();
    ...
    Assert.assertEquals(PublicMethod.marketCancelOrder(testAddress001, testKey001, orderId,
