 ...
 *
 * <p>Tests boundary conditions for delegating bandwidth/energy resources including zero amounts,
 * self-delegation, and insufficient balance.
 */
@Slf4j
public class DelegateResourceEdgeTest001 extends TronBaseTest {
  ...
  public void test01DelegateBandwidth() {
    ...
    Assert.assertTrue(
        receiverAccount.getAcquiredDelegatedFrozenV2BalanceForBandwidth() > 0,
        "Receiver should have acquired delegated bandwidth");
    logger.info(
        "Receiver acquired bandwidth: {}",
  ...
  public void test02DelegateEnergy() {
    ...
    Account receiverAccount = PublicMethod.queryAccount(receiverAddress, blockingStubFull);
    Assert.assertTrue(
        receiverAccount.getAccountResource().getAcquiredDelegatedFrozenV2BalanceForEnergy() > 0,
        "Receiver should have acquired delegated energy");
  }

  @Test(
      enabled = true,
      description = "Delegate zero amount should fail",
      groups = {"daily", "staking"})
  ...
  @Test(
      enabled = true,
      description = "Delegate to self should fail",
      groups = {"daily", "staking"})
  public void test04DelegateToSelf() {
    Assert.assertFalse(
        PublicMethod.delegateResourceV2(
            ownerAddress,
            TronConstants.ONE_TRX,
            TronConstants.FREEZE_BANDWIDTH,
            ownerAddress,
            ownerKeyStr,
            blockingStubFull),
  ...
  @Test(
      enabled = true,
      description = "Delegate more than frozen should fail",
      groups = {"daily", "staking"})
