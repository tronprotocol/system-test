...
public class WalletTestAssetIssue008 extends TronBaseTest {
  ...
  public void testGetAllAssetIssueFromSolidity() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            queryAssetIssueFromSoliAddress,
            2048000000,
            foundationAddress,
            foundationKey,
            blockingStubFull));
    ...
    GrpcAPI.AssetIssueList assetIssueList =
        blockingStubSolidity.getAssetIssueList(GrpcAPI.EmptyMessage.newBuilder().build());
    ...
    if (assetIssueList.getAssetIssueCount() == 0) {
      Assert.assertTrue(
          PublicMethod.freezeBalance(
              foundationAddress, 10000000L, 3, foundationKey, blockingStubFull));
      Assert.assertTrue(
          PublicMethod.sendcoin(
              toAddress, 999999L, foundationAddress, foundationKey, blockingStubFull));
      Block currentBlock = blockingStubFull.getNowBlock(GrpcAPI.EmptyMessage.newBuilder().build());
      logger.info(
          "fullnode block num is "
  ...
  public void aftertest() {
    PublicMethod.freeResource(
        queryAssetIssueFromSoliAddress, queryAssetIssueKey, foundationAddress, blockingStubFull);
  }
