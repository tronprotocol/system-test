...
@Slf4j
public class WalletTestZenToken004 extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            receiverPublicAddress, 1000000L, foundationAddress, foundationKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
  }

  @Test(
      enabled = false,
      description = "Shield to two shield transaction",
      groups = {"daily", "shield"})
  public void test1Shield2TwoShieldTransaction() {
    ...
    shieldOutList =
        PublicMethod.addShieldOutputList(
            shieldOutList, sendshieldAddress, "" + costTokenAmount, memo);
    Assert.assertTrue(
        PublicMethod.sendShieldCoin(
            foundationZenTokenAddress,
            costTokenAmount + zenTokenFee,
            null,
            null,
            shieldOutList,
            null,
  ...
  public void test3Public2OneShieldAndOnePublicTransaction() {
    ...
    Assert.assertEquals(receiverShieldTokenAmount2, sendToShiledAddress2Amount);
    Assert.assertEquals(memo2, PublicMethod.getMemo(note));
    final Long afterNotActivePublicAssetBalance =
        PublicMethod.getAssetIssueValue(
            notActivePublicAddress,
            PublicMethod.queryAccount(foundationZenTokenKey, blockingStubFull).getAssetIssuedID(),
            blockingStubFull);
