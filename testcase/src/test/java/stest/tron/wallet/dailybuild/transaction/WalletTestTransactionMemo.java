...
public class WalletTestTransactionMemo extends TronBaseTest {
  ...
  public void beforeClass() {
    PublicMethod.printAddress(memoKey);
    Assert.assertTrue(
  ...
  @Test(
      enabled = false,
      description = "Transaction with memo should be pay memo fee",
      groups = {"daily"})
  public void test01TransactionMemo() {
    ...
    String txid =
        PublicMethod.sendcoinWithMemoGetTransactionId(
    ...
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    Long noMemoFreeNet =
        PublicMethod.getTransactionInfoById(txidWithNoMemo, blockingStubFull)
            .get()
            .getReceipt()
            .getNetUsage();
  ...
  @Test(
      enabled = false,
      description = "transaction's max size is 500*1024",
      groups = {"daily"})
  public void test02TransactionMaxSize() {
    Assert.assertTrue(
        PublicMethod.sendcoinWithScript(
            memoAddress,
            10000000000L,
            foundationAddress,
            foundationKey,
