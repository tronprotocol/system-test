...
public class TvmAssetIssue003 extends TronBaseTest {
  ...
  @Test(
      enabled = false,
      description = "updateAsset illegal parameter verification",
      groups = {"contract", "daily"})
  public void updateAsset001IllegalParameterVerification() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            dev001Address, 1100_000_000L, fromAddress, foundationKey, blockingStubFull));
  ...
  @Test(
      enabled = false,
      description = "updateAsset called multiple times in one contract",
      groups = {"contract", "daily"})
  public void updateAsset002CalledMultipleTimesInOneContract() {
    ...
    Assert.assertEquals(abbr, ByteArray.toStr(assetIssueById.getAbbr().toByteArray()));
    Assert.assertEquals(
        description2, ByteArray.toStr(assetIssueById.getDescription().toByteArray()));
  ...
  @Test(
      enabled = false,
      description = "updateAsset revert",
      groups = {"contract", "daily"})
  public void updateAsset003Revert() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            dev001Address, 1500_000_000L, fromAddress, foundationKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
  ...
  @Test(
      enabled = false,
      description = "updateAsset call another contract in one contract",
      groups = {"contract", "daily"})
  ...
  @Test(
      enabled = false,
      description = "updateAsset verify token",
      groups = {"contract", "daily"})
  public void updateAsset005VerifyTokenId() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            dev002Address, 50_000_000L, fromAddress, foundationKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    String filePath = "./src/test/resources/soliditycode/tvmAssetIssue001.sol";
    String contractName = "tvmAssetIssue001";
    HashMap retMap = PublicMethod.getBycodeAbi(filePath, contractName);
    ...
    AssetIssueContract assetIssueByIdBefore =
        PublicMethod.getAssetIssueById("1000004", blockingStubFull);
    ...
    txid =
        PublicMethod.triggerContract(
            contractAddress,
            "updateOtherAccountAsset(string,string)",
            param,
            false,
            0,
            maxFeeLimit,
    ...
    AssetIssueContract assetIssueByIdAfter =
        PublicMethod.getAssetIssueById("1000004", blockingStubFull);
    String descAfter =
        assetIssueByIdBefore.getDescription().size() == 0
            ? ""
