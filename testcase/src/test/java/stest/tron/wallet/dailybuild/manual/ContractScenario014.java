...
@Slf4j
public class ContractScenario014 extends TronBaseTest {  byte[] contractAddress1 = null;
  ...
  @Test(enabled = true, description = "Triple trigger in smart contract", groups = {"daily"})
  public void testTripleTrigger() {
    ...
    Assert.assertTrue(PublicMethod.sendcoin(contract014Address, 50_000_000_000L, foundationAddress,
  ...
  String filePath1 = "./src/test/resources/soliditycode/contractScenario014.sol";
    HashMap retMap1 = PublicMethod.getBycodeAbi(filePath1, contractName);
  String code1 = retMap1.get("byteCode").toString();
  String abi1 = retMap1.get("abI").toString();
  String parame = "\"" + Base58.encode58Check(contractAddress1) + "\"";

    txid = PublicMethod.deployContractWithConstantParame(contractName, abi1, code1,
  ...
  String filePath2 = "./src/test/resources/soliditycode/contractScenario014.sol";
    HashMap retMap2 = PublicMethod.getBycodeAbi(filePath2, contractName);
  ...
  String abi2 = retMap2.get("abI").toString();
    ...
    txid = PublicMethod.deployContractWithConstantParame(contractName, abi2, code2,
        "constructor(address)", parame, "", maxFeeLimit, 1000000L, 100, null,
        contract014Key, contract014Address, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    infoById = PublicMethod.getTransactionInfoById(txid, blockingStubFull);
    ...
    Assert.assertTrue(PublicMethod.sendcoin(receiverAddress, 1000000L, foundationAddress, foundationKey,
        blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
  //Test contract2 trigger contract1 to test call function
    Account contract2AccountInfo = PublicMethod.queryAccount(contractAddress2, blockingStubFull);
  final Long contract2BeforeBalance = contract2AccountInfo.getBalance();
    Account receiverAccountInfo = PublicMethod.queryAccount(receiverAddress, blockingStubFull);
  Long receiverBeforeBalance = receiverAccountInfo.getBalance();
    Account contract1AccountInfo = PublicMethod.queryAccount(contractAddress1, blockingStubFull);
  ...
  String receiveAddress = "\"" + Base58.encode58Check(receiverAddress) + "\"";
    ...
    infoById = PublicMethod.getTransactionInfoById(txid, blockingStubFull);
    Assert.assertTrue(infoById.get().getResultValue() == 0);
    contract2AccountInfo = PublicMethod.queryAccount(contractAddress2, blockingStubFull);
  ...
  //contract2AccountInfo.getAccountResource().getFrozenBalanceForEnergy();
    receiverAccountInfo = PublicMethod.queryAccount(receiverAddress, blockingStubFull);
  Long receiverAfterBalance = receiverAccountInfo.getBalance();
    contract1AccountInfo = PublicMethod.queryAccount(contractAddress1, blockingStubFull);
  ...
  //Test contract3 trigger contract2 to call contract1
    contract1AccountInfo = PublicMethod.queryAccount(contractAddress1, blockingStubFull);
    contract1BeforeBalance = contract1AccountInfo.getBalance();
    Account contract3AccountInfo = PublicMethod.queryAccount(contractAddress3, blockingStubFull);
  final Long contract3BeforeBalance = contract3AccountInfo.getBalance();
    ...
    infoById = PublicMethod.getTransactionInfoById(txid, blockingStubFull);
    Assert.assertTrue(infoById.get().getResultValue() == 0);
    contract3AccountInfo = PublicMethod.queryAccount(contractAddress3, blockingStubFull);
  ...
  public void shutdown() throws InterruptedException {
    PublicMethod.unFreezeBalance(contract014Address, contract014Key, 1, contract014Address,
        blockingStubFull);
