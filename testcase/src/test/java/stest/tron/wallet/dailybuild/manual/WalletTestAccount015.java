...
public class WalletTestAccount015 extends TronBaseTest {
  ...
  public void test01SetAccountId() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account015Address, amount, foundationAddress, testKey002, blockingStubFull));
  ...
  public void test02GetAccountById() {
    Assert.assertEquals(
        amount, PublicMethod.getAccountById(accountId, blockingStubFull).getBalance());
  }

  @Test(
      enabled = true,
      description = "Get account by id from solidity",
      groups = {"daily"})
  public void test03GetAccountByIdFromSolidity() {
    Assert.assertEquals(
        amount,
        PublicMethod.getAccountByIdFromSolidity(accountId, blockingStubSoliInFull).getBalance());
  }

  @Test(
      enabled = true,
      description = "Get account by id from PBFT",
      groups = {"daily"})
  public void test04GetAccountByIdFromPbft() {
    Assert.assertEquals(
        amount, PublicMethod.getAccountByIdFromSolidity(accountId, blockingStubPbft).getBalance());
  }

  @Test(
      enabled = true,
      description = "Get account from PBFT",
      groups = {"daily"})
  public void test05GetAccountFromPbft() {
    Assert.assertEquals(
        amount, PublicMethod.queryAccount(account015Address, blockingStubPbft).getBalance());
  }

  @Test(
      enabled = true,
      description = "List witnesses",
      groups = {"daily"})
  public void test06ListWitness() {
    Assert.assertTrue(PublicMethod.listWitnesses(blockingStubFull).get().getWitnessesCount() >= 2);
  }

  @Test(
      enabled = true,
      description = "List witnesses from solidity node",
      groups = {"daily"})
  public void test07ListWitnessFromSolidity() {
    Assert.assertTrue(
        PublicMethod.listWitnessesFromSolidity(blockingStubSolidity).get().getWitnessesCount()
            >= 2);
    Assert.assertTrue(
        PublicMethod.listWitnessesFromSolidity(blockingStubSoliInFull).get().getWitnessesCount()
            >= 2);
  }

  @Test(
      enabled = true,
      description = "List witnesses from PBFT node",
      groups = {"daily"})
  public void test08ListWitnessFromPbft() {
    Assert.assertTrue(
        PublicMethod.listWitnessesFromSolidity(blockingStubPbft).get().getWitnessesCount() >= 2);
  }
  ...
  public void shutdown() throws InterruptedException {
    PublicMethod.freeResource(
        account015Address, account015Key, foundationAddress, blockingStubFull);
