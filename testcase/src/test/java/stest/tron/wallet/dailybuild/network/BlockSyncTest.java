 ...
 *
 * <p>Verifies that two nodes stay in sync: block heights should converge and the same block number
 * should have the same block hash on both nodes.
 */
@Slf4j
...
public class BlockSyncTest extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Block heights on two nodes should be close",
      groups = {"daily"})
  public void test01BlockHeightConvergence() {
    ...
    Assert.assertTrue(
        diff <= 3, "Block height difference between nodes should be <= 3, got " + diff);
  }

  @Test(
      enabled = true,
      description = "Same block number should have same content on both nodes",
      groups = {"daily"})
  public void test02SameBlockSameContent() {
    ...
    Assert.assertEquals(ts1, ts2, "Block " + targetNum + " timestamp should match on both nodes");

    long num1 = blockOnNode1.getBlockHeader().getRawData().getNumber();
    long num2 = blockOnNode2.getBlockHeader().getRawData().getNumber();
    Assert.assertEquals(num1, num2, "Block number should match on both nodes");

    String witness1 = blockOnNode1.getBlockHeader().getRawData().getWitnessAddress().toStringUtf8();
    String witness2 = blockOnNode2.getBlockHeader().getRawData().getWitnessAddress().toStringUtf8();
    Assert.assertEquals(witness1, witness2, "Block witness should match on both nodes");
  ...
  @Test(
      enabled = true,
      description = "New blocks appear on both nodes after waiting",
      groups = {"daily"})
  public void test03ContinuousSyncVerification() {
    ...
    logger.info(
        "Node1: {} -> {}, Node2: {} -> {}", startHeight1, endHeight1, startHeight2, endHeight2);
  }
