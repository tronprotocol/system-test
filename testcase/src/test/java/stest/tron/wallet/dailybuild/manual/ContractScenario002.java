...
@Slf4j
@MultiNode
public class ContractScenario002 extends TronBaseTest {
  ...
  public void beforeClass() {
    initPbftChannel();
    initSolidityChannel();
    PublicMethod.printAddress(contract002Key);
  ...
  @Test(
      enabled = true,
      description = "Deploy contract with java-tron support interface",
      groups = {"daily"})
  public void test01DeployTronNative() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            contract002Address, 500000000L, foundationAddress, foundationKey, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethod.freezeBalanceGetEnergy(
            contract002Address, 1000000L, 0, 1, contract002Key, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    AccountResourceMessage accountResource =
        PublicMethod.getAccountResource(contract002Address, blockingStubFull);
    ...
    Long balanceAfter =
        PublicMethod.queryAccount(contract002Address, blockingStubFull1).getBalance();
    ...
    PublicMethod.unFreezeBalance(
        contract002Address, contract002Key, 1, contract002Address, blockingStubFull);
  }

  /** constructor. */
  @Test(
      enabled = true,
      description = "Get smart contract with invalid address",
      groups = {"daily"})
  ...
  @Test(
      enabled = true,
      description = "Get transaction by id from solidity",
      groups = {"daily"})
  ...
  @Test(
      enabled = true,
      description = "Get transaction by id from PBFT",
      groups = {"daily"})
  public void test04GetTransactionByIdFromPbft() {
    Assert.assertFalse(
        PublicMethod.getTransactionById(txid, blockingStubPbft).get().getSignature(0).isEmpty());
    Assert.assertEquals(
        PublicMethod.getTransactionById(txid, blockingStubSoliInFull),
  ...
  public void test05GetTransactionInfoByIdFromSolidity() throws Exception {
    ...
    Assert.assertEquals(
        PublicMethod.getTransactionInfoByIdFromSolidity(txid, blockingStubSolidity)
            .get()
            .getReceipt()
            .getNetUsage(),
        netUsage);
    ...
    Assert.assertEquals(
        PublicMethod.getTransactionInfoByIdFromSolidity(txid, blockingStubSoliInFull)
            .get()
            .getReceipt()
            .getNetUsage(),
        netUsage);
  ...
  public void test06GetTransactionInfoByIdFromPbft() {
    ...
    Assert.assertEquals(
        PublicMethod.getTransactionInfoByIdFromSolidity(txid, blockingStubPbft)
            .get()
            .getReceipt()
            .getEnergyUsage(),
        energyUsage);
  ...
  public void shutdown() throws InterruptedException {
    PublicMethod.freeResource(
        contract002Address, contract002Key, foundationAddress, blockingStubFull);
