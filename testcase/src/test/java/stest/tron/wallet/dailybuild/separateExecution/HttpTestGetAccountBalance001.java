...
public class HttpTestGetAccountBalance001 {
  ...
  @Test(
      enabled = true,
      priority = 2,
      description = "Get account balance by http",
      groups = {"daily", "serial"})
  public void test01GetAccountBalance() {
    response =
        HttpMethod.getAccountBalance(
  ...
  @Test(
      enabled = true,
      priority = 2,
      description = "Get block balance by http",
      groups = {"daily", "serial"})
  public void test02GetBlockBalance() {
    response = HttpMethod.getBlockBalance(httpnode, sendcoinBlockNumber, sendcoinBlockHash);
    Assert.assertEquals(response.getStatusLine().getStatusCode(), 200);
    responseContent = HttpMethod.parseResponseContent(response);
    HttpMethod.printJsonContent(responseContent);
    Assert.assertTrue(responseContent.size() >= 2);
    ...
    response =
        HttpMethod.getBlockBalance(httpnode, deployContractBlockNumber, deployContractBlockHash);
    Assert.assertEquals(response.getStatusLine().getStatusCode(), 200);
    responseContent = HttpMethod.parseResponseContent(response);
    HttpMethod.printJsonContent(responseContent);
    Assert.assertTrue(responseContent.size() >= 2);
  ...
  @Test(
      enabled = false,
      priority = 2,
      description = "Get receipt root by http",
      groups = {"daily", "serial"})
