...
public class GetBlock001 extends JsonRpcBase {
  ...
  @Test(
      enabled = true,
      description = "Json rpc api of eth_getBlockByHash",
      groups = {"daily", "serial"})
  public void test01JsonRpcApiTestForEthGetBlockByHash() throws Exception {
    ...
    Assert.assertEquals(
        Long.toHexString(blockNum),
        responseContent.getJSONObject("result").getString("number").substring(2));
    Assert.assertEquals(
        blockId, responseContent.getJSONObject("result").getString("hash").substring(2));
    Assert.assertTrue(
        responseContent.getJSONObject("result").getJSONArray("transactions").size() >= 1);
  }
