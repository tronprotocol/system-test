...
public class ContractTrcToken035 extends TronBaseTest {
  ...
  @Test(enabled = true, description = "Deploy after transfertoken execute assert contract", groups = {"contract", "daily"})
  public void deploy01TransferTokenContract() {

    Assert.assertTrue(PublicMethod.sendcoin(dev001Address, 9999000000L, foundationAddress,
            testKey002, blockingStubFull));
    logger.info(
        "dev001Address:" + Base58.encode58Check(dev001Address));
    Assert.assertTrue(PublicMethod.sendcoin(user001Address, 4048000000L, foundationAddress,
            testKey002, blockingStubFull));
    logger.info(
  ...
  @Test(enabled = true, description = "Trigger after transfertoken execute assert contract", groups = {"contract", "daily"})
