...
@Slf4j
public class Opcode extends TronBaseTest {
  ...
  @Test(enabled = true, description = "test opcode smod, used for int", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test opcode extcodecopy return contract bytecode", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test opcode coinbase," 
      + "block.coinbase is sr address which produced the block", groups = {"contract", "daily"})
  ...
  @Test(enabled = false, description = "test opcode difficulty,block.difficulty is always 0,"
      + " Deprecate since 0.8.18", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test opcode gaslimit,block.gaslimit is always 0", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test opcode pc,return current position, " 
      + "ppppc() can refer to opCode.sol", groups = {"contract", "daily"})
  ...
  String abi = "[\n" 
        ...
        + "\t}\n" 
        + "]";
  byte[] temContract = PublicMethod.deployContract("A", abi, code, "", maxFeeLimit,
        0L, 100, null, contractExcKey,
        contractExcAddress, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    SmartContractOuterClass.SmartContract smartContract = PublicMethod.getContract(temContract,
        blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());
  ...
  @Test(enabled = true, description = "test opcode msize,return size of memory, " 
      + "msize cannot be used if optimize is open, mmmsize() can refer to opCode.sol", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test opcode swap14-16,solidity cannot use optimize", groups = {"contract", "daily"})
  ...
  @Test(enabled = true, description = "test opcode push13-30 but exclude push20 and push29," 
      + "solidity cannot use optimize", groups = {"contract", "daily"})
...
//            + "1223344556677889900112233"));
  ...
  @Test(enabled = true, description = "test opcode callcode,difference with delegatecall " 
      ...
      + "can refer to opCode.sol for code", groups = {"contract", "daily"})
  ...
  String abi = "[\n" 
        ...
        + "\t}\n" 
        + "]";
  byte[] contractA = PublicMethod.deployContract("A", abi, code, "", maxFeeLimit,
    ...
    abi = "[\n"
        ...
        + "\t}\n"
        + "]";
  byte[] contractC = PublicMethod.deployContract("C", abi, code, "", maxFeeLimit,
        0L, 100, null, contractExcKey,
        contractExcAddress, blockingStubFull);
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    smartContract = PublicMethod.getContract(contractC,
        blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());
  ...
  String x = ByteArray.toHexString(contractA);
    ...
    txid = PublicMethod.triggerContract(contractA,
