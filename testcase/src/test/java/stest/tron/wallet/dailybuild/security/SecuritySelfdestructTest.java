 ...
 *
 * <p>TIP-481 ({@code getAllowTvmSelfdestructRestriction}) is a chain governance proposal (proposal
 * #76) that restricts selfdestruct behavior. When active:
 *
 * <ul>
 *   <li>selfdestruct only transfers TRX to the target, does NOT delete code/storage
 *   <li>Contract remains callable after selfdestruct
 *   <li>This aligns with Ethereum's EIP-6780 (Cancun hard fork)
 * </ul>
 *
 * <p>This proposal is enabled via on-chain transaction (not config file), typically opened in
 * {@code @BeforeSuite} by the test framework (see JsonRpcBase).
 */
@Slf4j
public class SecuritySelfdestructTest extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            testAddress, 500_000_000_000L, foundationAddress, foundationKey, blockingStubFull));
  ...
  public void test01SelfdestructTransfersTrx() {
    long targetBalanceBefore =
        PublicMethod.queryAccount(targetAddress, blockingStubFull).getBalance();
    long contractBalance =
        PublicMethod.queryAccount(selfdestructAddress, blockingStubFull).getBalance();
  ...
  public void test04FactoryCreateChild() {
    ...
    Assert.assertTrue(
        "Should have internal transactions", info.get().getInternalTransactionsCount() > 0);
    byte[] childAddress =
        info.get().getInternalTransactions(0).getTransferToAddress().toByteArray();
