...
@Slf4j
public class WalletTestZenToken001 extends TronBaseTest {
  ...
  @Test(
      enabled = false,
      description = "Public to shield transaction",
      groups = {"daily", "shield"})
  ...
  @Test(
      enabled = false,
      description = "Shield to public transaction",
      groups = {"daily", "shield"})
  public void test2Shield2PublicTransaction() {
    note = notes.getNoteTxs(0).getNote();
    SpendResult result =
        PublicMethod.getSpendResult(shieldAddressInfo.get(), notes.getNoteTxs(0), blockingStubFull);
  ...
  public void test3Shield2PublicAmountIsZero() {
    ...
    shieldOutList =
        PublicMethod.addShieldOutputList(
            shieldOutList,
            shieldAddress,
            "" + (note.getValue() - zenTokenFee - (zenTokenFee - note.getValue())),
            memo);
    Assert.assertFalse(
        PublicMethod.sendShieldCoin(
            null,
    ...
    shieldOutList =
        PublicMethod.addShieldOutputList(
            shieldOutList, shieldAddress, "" + (note.getValue() - zenTokenFee), memo);
    ...
    shieldOutList.clear();
    shieldOutList =
        PublicMethod.addShieldOutputList(shieldOutList, shieldAddress, "" + (-zenTokenFee), memo);
    Assert.assertFalse(
        PublicMethod.sendShieldCoin(
            null,
            0,
