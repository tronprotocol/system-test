...
public class SecurityReentrancyTest extends TronBaseTest {
  ...
  @Test(
      enabled = true,
      description = "Deposit TRX into victim contract",
      groups = {"daily"})
  ...
  @Test(
      enabled = true,
      description = "Normal withdraw from victim should succeed",
      groups = {"daily"})
  ...
  public void test03ReentrancyAttack() {
    ...
    long victimBalanceBefore =
        PublicMethod.queryAccount(victimAddress, blockingStubFull).getBalance();
  ...
  @Test(
      enabled = true,
      description = "Safe withdraw with checks-effects-interactions pattern",
      groups = {"daily"})
