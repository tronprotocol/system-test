...
public class WalletTestCommittee004 extends TronBaseTest {
  ...
  public void test1DeleteProposal() {
    PublicMethod.sendcoin(witness001Address, 1000000L, toAddress, testKey003, blockingStubFull);
    PublicMethod.sendcoin(witness002Address, 1000000L, toAddress, testKey003, blockingStubFull);
    ...
    Assert.assertTrue(
        PublicMethod.approveProposal(
            witness001Address, witnessKey, proposalId, true, blockingStubFull));
    ...
    Assert.assertFalse(
        PublicMethod.deleteProposal(witness002Address, witnessKey2, proposalId, blockingStubFull));
    // Cancel the proposal
    Assert.assertTrue(
        PublicMethod.deleteProposal(witness001Address, witnessKey, proposalId, blockingStubFull));
    // When the state is cancel, you can't delete it again.
    Assert.assertFalse(
        PublicMethod.deleteProposal(witness001Address, witnessKey, proposalId, blockingStubFull));
    // You can't delete an invalid proposal
    Assert.assertFalse(
        PublicMethod.deleteProposal(
            witness001Address, witnessKey, proposalId + 100, blockingStubFull));
    PublicMethod.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertFalse(
        PublicMethod.approveProposal(
            witness001Address, witnessKey, proposalId, false, blockingStubFull));
  }
  ...
  public void test2GetProposal() {
    ...
    BytesMessage request =
        BytesMessage.newBuilder()
            .setValue(
                ByteString.copyFrom(ByteArray.fromLong(Long.parseLong(proposalId.toString()))))
            .build();
    ...
    request =
        BytesMessage.newBuilder()
            .setValue(
                ByteString.copyFrom(ByteArray.fromLong(Long.parseLong(wrongProposalId.toString()))))
            .build();
  ...
  public void testGetChainParameters() {
    ...
    Assert.assertTrue(getChainParameters.get().getChainParameter(1).getValue() == 9999000000L);
    Assert.assertTrue(getChainParameters.get().getChainParameter(4).getValue() == 1024000000L);
