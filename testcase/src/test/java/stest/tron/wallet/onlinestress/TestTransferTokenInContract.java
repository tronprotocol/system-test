...
public class TestTransferTokenInContract extends TronBaseTest {
  ...
  @Test(enabled = true, threadPoolSize = 2, invocationCount = 2, groups = {"stress"})
  ...
  String user001Key = ByteArray.toHexString(ecKey2.getPrivKeyBytes());
    ...
    Assert
        .assertTrue(PublicMethod.sendcoin(dev001Address, 2048000000, foundationAddress,
  ...
  byte[] receiveTokenContractAddress = PublicMethod
        .deployContract(contractName, abi, code, "", maxFeeLimit,
  ...
  byte[] tokenBalanceContractAddress = PublicMethod
        .deployContract(contractName, abi, code, "", maxFeeLimit,
            0L, 100, null, dev001Key, dev001Address, blockingStubFull);
  // devAddress transfer token to userAddress
    PublicMethod.transferAsset(user001Address, tokenId.toByteArray(), 100, dev001Address, dev001Key,
        blockingStubFull);
    ...
    for (int i = 0; i < 1000000000000L; i++) {
      ...
      PublicMethod.freezeBalanceForReceiver(user001Address,
          PublicMethod.getFreezeBalanceCount(user001Address,
              user001Key, 300000L, blockingStubFull), 0,
          1, ByteString.copyFrom(foundationAddress), testKey002, blockingStubFull);
      PublicMethod.freezeBalanceForReceiver(user001Address, 10_000_000L,
          0, 0, ByteString.copyFrom(foundationAddress), testKey002, blockingStubFull);
      try {
        Thread.sleep(3000);
  ...
  String triggerTxid = PublicMethod.triggerContract(transferTokenContractAddress,
      ...
      PublicMethod.triggerContract(tokenBalanceContractAddress, "balance()",
  ...
  // user trigger C to get devAddress's token balance
      ...
      PublicMethod.unFreezeBalance(foundationAddress, user001Key, 1,
          user001Address, blockingStubFull);
      PublicMethod.unFreezeBalance(foundationAddress, user001Key, 0,
          user001Address, blockingStubFull);
    }
  }
