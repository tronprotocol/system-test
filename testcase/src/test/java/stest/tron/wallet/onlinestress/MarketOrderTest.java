  ...
  public void markerOrderCancle() {
    ...
    while (true) {
      ...
      for (String key : addressList) {
        byte[] addressByteArray = PublicMethod.getFinalAddress(key);
        List<MarketOrder> OrderList =
            PublicMethod.getMarketOrderByAccount(addressByteArray, blockingStubFull)
                .get()
                .getOrdersList();
        int single_times = 0;
        for (MarketOrder order : OrderList) {
          ...
          String txid =
              PublicMethod.marketCancelOrder(addressByteArray, key, orderId, blockingStubFull);
          if (single_times++ >= 2) {
            break;
          }
        }
