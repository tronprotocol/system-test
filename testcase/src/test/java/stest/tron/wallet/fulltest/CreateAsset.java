  ...
  public void createAssetissue() throws InterruptedException {
    ...
    while (fromAccountInfo.getBalance() > 1025000000) {
      ...
      PublicMethod.sendcoin(
          asset016Address, sendAmount, foundationAddress, testKey002, blockingStubFull);
      ...
      if (ret) {
        ...
        //   .getAssetIssueList(GrpcAPI.EmptyMessage.newBuilder().build());
        // logger.info("assetissue num is " +
        // Integer.toString(assetIssueList.getAssetIssueCount()));
        try {
        ...
        participateRet =
            PublicMethod.participateAssetIssue(
                asset016Address, name.getBytes(), 1L, toAddress, testKey003, blockingStubFull);
        if (participateRet) {
