...
public class TvmContract extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            contract008Address, 500000000L, foundationAddress, testKey002, blockingStubFull));
    logger.info(
        Long.toString(PublicMethod.queryAccount(contract008Key, blockingStubFull).getBalance()));
    Assert.assertTrue(
        PublicMethod.freezeBalanceGetEnergy(
            contract008Address, 1000000L, 3, 1, contract008Key, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.buyStorage(50000000L, contract008Address, contract008Key, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.freezeBalance(
            contract008Address, 5000000L, 3, contract008Key, blockingStubFull));
  }
  ...
  public void deployErc721CryptoKitties() {
    ...
    for (Integer i = 0; i < 1; i++) {
      ...
      logger.info(
          "Time " + Integer.toString(i) + ": free net used is " + Long.toString(freeNet - net));
