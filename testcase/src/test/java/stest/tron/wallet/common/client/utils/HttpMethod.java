...
public class HttpMethod {
  ...
  public static HttpResponse setAccountId(
    ...
    try {
      ...
      userBaseObj2.addProperty(
          "owner_address", Base58.encode58Check(PublicMethod.getFinalAddress(fromKey)));
  ...
  public static String sendCoin(
    ...
    JSONObject responseContent = HttpMethod.parseStringContent(transactionString);
  ...
  public static String triggerContractGetTxid(
    ...
    JSONObject responseContent = HttpMethod.parseStringContent(transactionSignString);
    return responseContent.getString("txID");
  }
  ...
  public static void waitToProduceOneBlock(String httpNode) {
    HttpResponse response = HttpMethod.getNowBlock(httpNode);
    JSONObject responseContent = HttpMethod.parseResponseContent(response);
    ...
    while (nextBlockNum <= currentBlockNum + 1 && times++ <= 10) {
      ...
      responseContent = HttpMethod.parseResponseContent(response);
      responseContent =
          HttpMethod.parseStringContent(responseContent.get("block_header").toString());
      responseContent = HttpMethod.parseStringContent(responseContent.get("raw_data").toString());
  ...
  public static void waitToProduceOneBlockFromPbft(String httpNode, String httpSolidityNode) {
    ...
    while (nextBlockNum <= currentBlockNum && times++ <= 3) {
      response = HttpMethod.getNowBlockFromPbft(httpSolidityNode);
      responseContent = HttpMethod.parseResponseContent(response);
      responseContent =
          HttpMethod.parseStringContent(responseContent.get("block_header").toString());
      responseContent = HttpMethod.parseStringContent(responseContent.get("raw_data").toString());
  ...
  public static Long getAssetIssueValue(
    ...
    HttpResponse response = HttpMethod.getAccount(httpNode, accountAddress);
    JSONObject responseContent = HttpMethod.parseResponseContent(response);
    ...
    if (assetV2 != null && assetV2.size() > 0) {
      for (int i = 0; i < assetV2.size(); i++) {
        ...
        String assetKey = HttpMethod.parseStringContent(assetString).getString("key");
        if (assetKey.equals(assetIssueId)) {
          return HttpMethod.parseStringContent(assetString).getLong("value");
        }
      }
    }
    return 0L;
  ...
  public static void disConnect() {
    // No-op: connections are managed by the pooling connection manager.
    // Individual requests use local HttpPost variables that go out of scope.
  }
  ...
  public static Optional<ShieldAddressInfo> generateShieldAddress(String httpnode) {
    ...
    String pkD;
    try {
      HttpResponse response = HttpMethod.getSpendingKey(httpnode);
      JSONObject responseContent = HttpMethod.parseResponseContent(response);
      ...
      response = HttpMethod.getZenPaymentAddress(httpnode, ivk, d);
      responseContent = HttpMethod.parseResponseContent(response);
  ...
  public static List<GrpcAPI.Note> addShieldOutputList(
    ...
    HttpResponse response = HttpMethod.getRcm(httpNode);
    JSONObject responseContent = HttpMethod.parseResponseContent(response);
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        HttpResponse expandedSpendingKey =
            HttpMethod.getExpandedSpendingKey(
                httpNode, ByteArray.toHexString(shieldAddressInfo.getSk()));
        JSONObject responseContent = HttpMethod.parseResponseContent(expandedSpendingKey);
        HttpMethod.printJsonContent(responseContent);
        ...
        response =
            HttpMethod.getMerkleTreeVoucherInfo(httpNode, noteTx.getTrxId(), noteTx.getIndex(), 1);
        responseContent = HttpMethod.parseResponseContent(response);
  ...
  public static List<ShieldNoteInfo> scanNoteByOvkFromPbft(
    ...
    try {
      HttpResponse response = HttpMethod.getNowBlockFromPbft(httpSolidityNode);
      JSONObject responseContent = HttpMethod.parseResponseContent(response);
      HttpMethod.printJsonContent(responseContent);
      String blockHeaderString = responseContent.getString("block_header");
      String rawDataString =
          HttpMethod.parseStringContent(blockHeaderString).get("raw_data").toString();
      Integer currentBlockNum = HttpMethod.parseStringContent(rawDataString).getInteger("number");
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        String noteString = HttpMethod.parseStringContent(noteTxs).getString("note");
        noteTx.setValue(HttpMethod.parseStringContent(noteString).getLong("value"));
        noteTx.setPaymentAddress(
            HttpMethod.parseStringContent(noteString).getString("payment_address"));
        noteTx.setR(
            ByteArray.fromHexString(HttpMethod.parseStringContent(noteString).getString("rcm")));
        noteTx.setMemo(
            ByteArray.fromHexString(HttpMethod.parseStringContent(noteString).getString("memo")));
        noteTx.setTrxId(HttpMethod.parseStringContent(noteTxs).getString("txid"));
        noteTx.setIndex(HttpMethod.parseStringContent(noteTxs).getIntValue("index"));
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        noteTx.setMemo(
            ByteArray.fromHexString(HttpMethod.parseStringContent(noteString).getString("memo")));
        noteTx.setTrxId(HttpMethod.parseStringContent(noteTxs).getString("txid"));
        noteTx.setIsSpend(HttpMethod.parseStringContent(noteTxs).getBoolean("is_spend"));
        noteTx.setIndex(HttpMethod.parseStringContent(noteTxs).getIntValue("index"));
        shieldNoteInfoList.add(noteTx);
      }
      return shieldNoteInfoList;
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        HttpResponse expandedSpendingKey =
            HttpMethod.getExpandedSpendingKey(
                httpNode, ByteArray.toHexString(shieldAddressInfo.getSk()));
        responseContent = HttpMethod.parseResponseContent(expandedSpendingKey);
        HttpMethod.printJsonContent(responseContent);
        map.put("nsk", responseContent.getString("nsk"));
        ...
        response =
            HttpMethod.getMerkleTreeVoucherInfoFromPbft(
      ...
      if (shieldAddressInfo != null) {
        ...
        JSONObject jsonObject = HttpMethod.parseStringContent(transactionString);
        jsonObject
            .getJSONObject("raw_data")
  ...
  public static void freeResource(
    ...
    long balance = HttpMethod.getBalance(httpNode, fromAddress);
    // System.out.println("剩余资源：" + balance);
    sendCoin(httpNode, fromAddress, toAddress, balance - 50000, fromKey);
    balance = HttpMethod.getBalance(httpNode, fromAddress);
