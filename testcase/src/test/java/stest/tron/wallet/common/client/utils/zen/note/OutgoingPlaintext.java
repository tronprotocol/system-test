...
public class OutgoingPlaintext {
  ...
  @Getter @Setter private byte[] pkD;
  @Getter @Setter private byte[] esk;
  ...
  private static OutgoingPlaintext decode(Encryption.OutPlaintext outPlaintext) {
    ...
    OutgoingPlaintext ret =
        new OutgoingPlaintext(
            new byte[ZenChainParams.ZC_JUBJUB_SCALAR_SIZE],
            new byte[ZenChainParams.ZC_JUBJUB_POINT_SIZE]);
    System.arraycopy(data, 0, ret.pkD, 0, ZenChainParams.ZC_JUBJUB_SCALAR_SIZE);
    System.arraycopy(
        data,
        ZenChainParams.ZC_JUBJUB_SCALAR_SIZE,
        ret.esk,
        0,
        ZenChainParams.ZC_JUBJUB_POINT_SIZE);
    return ret;
  }

  public static Optional<OutgoingPlaintext> decrypt(
      Encryption.OutCiphertext ciphertext, byte[] ovk, byte[] cv, byte[] cm, byte[] epk)
      throws ZksnarkException {
    Optional<OutPlaintext> pt = Encryption.attemptOutDecryption(ciphertext, ovk, cv, cm, epk);
    if (!pt.isPresent()) {
