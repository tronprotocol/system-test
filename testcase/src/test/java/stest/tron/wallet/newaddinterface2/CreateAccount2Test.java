...
public class CreateAccount2Test extends TronBaseTest {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account007Address, 10000000, fromAddress, foundationKey, blockingStubFull));
  }
  ...
  public void testCreateAccount2() {
    ...
    AccountNetMessage accountNetInfo =
        PublicMethod.getAccountNet(account007Address, blockingStubFull);
    final Long beforeFreeNet = accountNetInfo.getFreeNetUsed();
    GrpcAPI.Return ret1 =
        PublicMethod.createAccount2(
            account007Address, newAccountAddress, account007Key, blockingStubFull);
    ...
    accountNetInfo = PublicMethod.getAccountNet(account007Address, blockingStubFull);
  ...
  public void testExceptionCreateAccount2() {
    ...
    GrpcAPI.Return ret1 =
        PublicMethod.createAccount2(
            account007Address, account007Address, account007Key, blockingStubFull);
    Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.CONTRACT_VALIDATE_ERROR);
    Assert.assertEquals(
        ret1.getMessage().toStringUtf8(), "Contract validate error : Account has existed");
    // Try to create an invalid account
    byte[] wrongAddress = "wrongAddress".getBytes();
    ret1 =
        PublicMethod.createAccount2(
            account007Address, wrongAddress, account007Key, blockingStubFull);
    Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.CONTRACT_VALIDATE_ERROR);
    Assert.assertEquals(
        ret1.getMessage().toStringUtf8(), "Contract validate error : Invalid account address");
  }
}
