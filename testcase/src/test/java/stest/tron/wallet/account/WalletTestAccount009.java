...
public class WalletTestAccount009 extends TronBaseTest {
  ...
  public void testGetEnergy() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account009Address, 10000000, foundationAddress, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account009SecondAddress, 10000000, foundationAddress, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account009InvalidAddress,
            10000000,
            foundationAddress,
            foundationKey,
            blockingStubFull));
    ...
    Assert.assertTrue(
        account009Info.getAccountResource().getFrozenBalanceForEnergy().getExpireTime() == 0);

    Assert.assertTrue(
        PublicMethod.freezeBalanceGetEnergy(
            account009Address, 1000000L, 3, 1, account009Key, blockingStubFull));
    account009Info = PublicMethod.queryAccount(account009Key, blockingStubFull);
    Assert.assertTrue(account009Info.getAccountResource().getEnergyUsage() == 0);
    Assert.assertTrue(
        account009Info.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance()
            == 1000000L);

    AccountResourceMessage account009Resource =
        PublicMethod.getAccountResource(account009Address, blockingStubFull);
  ...
  public void testGetEnergyInvalid() {
    ...
    Assert.assertTrue(
        PublicMethod.freezeBalanceGetEnergy(
            account009InvalidAddress, 1000000L, 3, 0, account009InvalidKey, blockingStubFull));
    Assert.assertFalse(
        PublicMethod.freezeBalanceGetEnergy(
            account009InvalidAddress, 1000000L, 3, -1, account009InvalidKey, blockingStubFull));
    Assert.assertFalse(
        PublicMethod.freezeBalanceGetEnergy(
            account009InvalidAddress, 1000000L, 3, 3, account009InvalidKey, blockingStubFull));
  }
