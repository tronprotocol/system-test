...
public class WalletTestAccount006 extends TronBaseTest {
  ...
  public void test1GetAccountNet() {
    ...
    Assert.assertTrue(
        PublicMethod.freezeBalance(
            foundationAddress, 100000000, 3, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account006Address, sendAmount, foundationAddress, foundationKey, blockingStubFull));
  ...
  public void test2UseFreeNet() {
    ...
    Assert.assertTrue(
        PublicMethod.sendcoin(
            foundationAddress, 1L, account006Address, account006Key, blockingStubFull));
    ByteString addressBs = ByteString.copyFrom(account006Address);
    Account request = Account.newBuilder().setAddress(addressBs).build();
    AccountNetMessage accountNetMessage = blockingStubFull.getAccountNet(request);
    // Every transaction may cost 200 net.
    Assert.assertTrue(
        accountNetMessage.getFreeNetUsed() > 0 && accountNetMessage.getFreeNetUsed() < 300);
  ...
  public void test3UseMoneyToDoTransaction() {
    Assert.assertTrue(
        PublicMethod.sendcoin(
            account006Address, 1000000L, foundationAddress, foundationKey, blockingStubFull));
    ByteString addressBs = ByteString.copyFrom(account006Address);
    Account request = Account.newBuilder().setAddress(addressBs).build();
    AccountNetMessage accountNetMessage = blockingStubFull.getAccountNet(request);
  ...
  public void test4UseNet() {
    ...
    Assert.assertTrue(
        PublicMethod.freezeBalance(
            account006Address, 10000000L, 3, account006Key, blockingStubFull));
    Assert.assertTrue(
        PublicMethod.sendcoin(toAddress, 1L, account006Address, account006Key, blockingStubFull));
    ByteString addressBs = ByteString.copyFrom(account006Address);
    Account request = Account.newBuilder().setAddress(addressBs).build();
    AccountNetMessage accountNetMessage = blockingStubFull.getAccountNet(request);
    Assert.assertTrue(accountNetMessage.getNetLimit() > 0);
    ...
    logger.info(
        "before is "
            + Long.toString(beforeSendBalance)
            + " and after is "
