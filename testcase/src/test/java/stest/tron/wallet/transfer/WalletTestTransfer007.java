...
@Slf4j
@MultiNode
public class WalletTestTransfer007 extends TronBaseTest {
  ...
  private ManagedChannel channelSolidityInFullnode =
      null; /*  private String solidityInFullnode = Configuration.getByPath("testng.conf")
  ...
  public void beforeClass() {
    ...
    /*
     * channelSolidityInFullnode =
     *     ManagedChannelBuilder.forTarget(solidityInFullnode)
     *         .usePlaintext()
     *         .build();
     * blockingStubSolidityInFullnode = WalletSolidityGrpc
     *     .newBlockingStub(channelSolidityInFullnode);
     */
  }
  ...
  public void testSendCoin() {
    ...
    String transactionId =
        PublicMethod.sendcoinGetTransactionId(
            sendAccountAddress, 90000000000L, foundationAddress2, foundationKey2, blockingStubFull);
    Optional<Transaction> infoById =
        PublicMethod.getTransactionById(transactionId, blockingStubFull);
    Long timestampFromTxid = PublicMethod.printTransactionRow(infoById.get().getRawData());
    Long timestampFromBlockOne =
        PublicMethod.getBlock(1, blockingStubFull).getBlockHeader().getRawData().getTimestamp();
    Assert.assertTrue(timestampFromTxid >= timestampFromBlockOne);
  }
