...
public class witnessTransaction extends TestBase {
  ...
  public void test02CreateWitness() throws Exception {
    //key for test
    ECKey newAccount = new ECKey();
    ApiWrapper wrapper = new ApiWrapper("grpc.nile.trongrid.io:50051",
        "grpc.nile.trongrid.io:50061", newAccount.toStringWithPrivate());
    Response.TransactionExtention transactionExtention = wrapper.createWitness(Base58.encode58Check(newAccount.getAddress()),"trident create witness");
    Transaction transaction = wrapper.signTransaction(transactionExtention);
    try {
      String broadcast = wrapper.broadcastTransaction(transaction);
    }catch (Exception e) {
  ...
  public void test03UpdateWitness() throws Exception {
    ...
    ApiWrapper wrapper = new ApiWrapper("grpc.nile.trongrid.io:50051",
        "grpc.nile.trongrid.io:50061", newAccount.toStringWithPrivate());
    Response.TransactionExtention transactionExtention = wrapper.updateWitness(Base58.encode58Check(newAccount.getAddress()),"update trident create witness");
    Transaction transaction = wrapper.signTransaction(transactionExtention);
    ;
    String broadcast = wrapper.broadcastTransaction(transaction);
